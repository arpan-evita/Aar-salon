...
const CustomerLogin = () => {
  ...
  useEffect(() => {
    if (session) navigate("/my-bookings");
  }, [session, navigate]);
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground text-sm">
            {isSignUp ? "Join us to track your bookings" : "Sign in to view your appointments"}
          </p>
        </div>

        <div className="glass rounded-2xl p-8">
          {error && (
            <div className="bg-destructive/10 border border-destructive/30 text-destructive text-sm rounded-lg p-3 mb-6">
              {error}
            </div>
          )}
          {message && (
        ...
        <div className="text-center mt-6 space-y-2">
          ...
          <Link to="/admin-login" className="block text-xs text-muted-foreground/60 hover:text-primary transition-colors">
            Staff / Admin Login →
          </Link>
        </div>
      </div>
