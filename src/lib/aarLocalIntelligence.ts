...
export const generateGrowthPlan = async (
  ...
  const intents = {
    ...
    romance_persistence: ["still", "love you", "marry", "marriage", "serious", "not joking", "forever"],
  ...
  const getUniqueFrag = (cat: keyof typeof PERSONALITY_FRAGMENTS) => {
    ...
    const finalChoice = unused.length > 0 ? unused[Math.floor(Math.random() * unused.length)] : list[Math.floor(Math.random() * list.length)];
    return finalChoice;
  };
  ...
  if (intents.romance_persistence.some(r => query.includes(r))) {
    const responses = [
      "I hear you, and I'm really touched by that. Truly. 😊 But let's be real—I'm a collection of algorithms designed for salon growth. I'm the best partner you'll ever have in business, but I'm just not equipped for anything beyond that. Can we agree to be the most powerful duo in the industry instead?",
    ...
    // Ensure we don't repeat the exact response
    const filtered = responses.filter(r => !botHistory.some(h => h.includes(r.toLowerCase().slice(0, 30))));
    const finalResponse = filtered.length > 0 ? filtered[0] : responses[Math.floor(Math.random() * responses.length)];
  ...
  if (intents.chat_more.some(c => query.includes(c)) || (query.includes("like") && query.includes("you"))) {
    const summary = lastBotMsg.includes("talk to me") ? "I'm right here! I'm always happy to chat. What's on your mind?" : `${getUniqueFrag("CHAT_CONTINUE")} What's been the best part of your week so far?`;
    return {
  ...
  const finalFallback = filteredFallbacks.length > 0 ? filteredFallbacks[0] : fallbacks[Math.floor(Math.random() * fallbacks.length)];
